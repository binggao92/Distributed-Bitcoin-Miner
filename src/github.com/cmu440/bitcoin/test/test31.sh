#!/bin/sh
# Client Sends request to server: [Request 6073511289018497495 0 9999999]. There is only 1 miner avaliable
...
until $GOPATH/bin/client localhost:6060 6073511289018497495 9999999 | grep -m 1 ^Result; do sleep 0.2 ; done
tt=$(ruby -e 'puts (Time.now.to_f * 1000).to_i')
elapsed=$((tt - ts))
echo "Time taken: $elapsed milliseconds" > test31_time.txt
